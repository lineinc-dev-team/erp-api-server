...
public record MaterialManagementChangeHistoryResponse(
        ...
        @Schema(description = "생성 일시",
                example = "2025-07-15T10:00:00+09:00") OffsetDateTime createdAt,
        @Schema(description = "수정 일시",
                example = "2025-07-15T10:00:00+09:00") OffsetDateTime updatedAt,
        @Schema(description = "수정자", example = "관리자") String updatedBy,
        @Schema(description = "변경 유형", example = "기본정보") String type,
        @Schema(description = "변경 유형 코드",
                example = "BASIC") MaterialManagementChangeHistoryType typeCode,
        @Schema(description = "수정 가능여부", example = "true") Boolean isEditable) {
    public static MaterialManagementChangeHistoryResponse from(
            final MaterialManagementChangeHistory history, final Long loginUserId) {
        return new MaterialManagementChangeHistoryResponse(history.getId(),
                history.getDescription(), history.getChanges(), history.getMemo(),
                history.getCreatedAt(), history.getUpdatedAt(), history.getUpdatedBy(),
                history.getType() != null ? history.getType().getLabel() : null, history.getType(),
