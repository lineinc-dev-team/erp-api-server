...
public class BatchScheduler {
    ...
    /**
     * 매일 새벽 00시 10분에 출역일보 자동 마감 배치 실행
     * Cron 표현식: "0 10 0 * * ?" (한국시간 새벽 00시 10분)
     */
    ...
    /**
     * 매월 2일 새벽 00시 20분에 퇴직금 발생 여부 계산 배치 실행
     * Cron 표현식: "0 20 0 2 * ?" (한국시간 매월 2일 새벽 00시 20분)
     */
    @Scheduled(cron = "0 20 0 2 * ?", zone = "Asia/Seoul")
    public void calculateSeverancePayEligibility() {
        executeBatchWithHistory(severancePayEligibilityBatchService);
    }
    ...
    public void executeBatchWithHistory(final BatchService batchService) {
        ...
        try {
            ...
            log.info("{} 완료 - 실행 시간: {}초", batchService.getBatchName().getLabel(), history.getExecutionTimeSeconds());
        } catch (final Exception e) {
            history.markAsFailed(e.getMessage());
            batchExecutionHistoryRepository.save(history);

            log.error("{} 실행 중 오류 발생 - 실행 시간: {}초", batchService.getBatchName().getLabel(),
                    history.getExecutionTimeSeconds(), e);
        }
    }
}
