...
@Schema(description = "강재수불부 V2 상세 응답")
public record SteelManagementV2DetailResponse(
    ...
    public static SteelManagementV2DetailResponse from(
            ...
            final SteelManagementDetailV2Type typeFilter) {
        // 타입 필터링 (optional)
        final List<SteelManagementDetailV2Response> details = entity.getDetails().stream()
                .filter(detail -> typeFilter == null || detail.getType() == typeFilter)
                .map(SteelManagementDetailV2Response::from)
                .sorted(Comparator.comparing(SteelManagementDetailV2Response::id))
                .toList();

        return new SteelManagementV2DetailResponse(
                entity.getId(),
