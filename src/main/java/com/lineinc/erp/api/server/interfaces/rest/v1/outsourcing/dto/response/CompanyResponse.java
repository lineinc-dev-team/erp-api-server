...
public record CompanyResponse(
    ...
    public static CompanyResponse from(
        ...
        return new CompanyResponse(company.getId(), company.getName(), company.getBusinessNumber(),
                company.getType() != null ? company.getType().getLabel() : null, company.getType(),
                company.getCeoName(), company.getAddress(), company.getDetailAddress(), company.getLandlineNumber(),
                company.getPhoneNumber(), company.getEmail(), company.isActive(),
                company.getDefaultDeductions() != null && !company.getDefaultDeductions().isEmpty()
                        ...
                        : null,
                company.getDefaultDeductions(), company.getDefaultDeductionsDescription(), company.getMemo(),
                company.getCreatedAt(), company.getUpdatedAt(),
                company.getFiles().stream().anyMatch(file -> file.getFileUrl() != null && !file.getFileUrl().isBlank()),
                company.getContacts()
                        .stream()
    ...
    public static record CompanySimpleResponse(
        ...
        public static CompanyResponse.CompanySimpleResponse from(
                final OutsourcingCompany company) {
            return new CompanyResponse.CompanySimpleResponse(company.getId(), company.getName(),
                    company.getBusinessNumber(), company.getType() != null ? company.getType().getLabel() : null,
                    company.getType(), company.getCeoName(), company.getBankName(), company.getAccountNumber(),
                    company.getAccountHolder(), company.getAddress(), company.getDetailAddress(),
                    company.getLandlineNumber(), company.getPhoneNumber(), company.isDeleted());
        }
    }
