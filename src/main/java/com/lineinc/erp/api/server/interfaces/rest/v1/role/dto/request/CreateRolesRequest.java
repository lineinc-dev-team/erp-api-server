...
public record CreateRolesRequest(
        ...
        @Schema(description = "계정 및 메모 리스트", example = "[{\"userId\": 1, \"memo\": \"현장 책임자\"}]") List<UserWithMemo> users,
    ...
    public static record UserWithMemo(
            ...
            @Schema(description = "비고 / 메모", example = "현장 책임자") String memo) {
    }

    public static record SiteProcessAccess(
            @Schema(description = "현장 ID", example = "1") Long siteId,

            @Schema(description = "공정 ID", example = "3") Long processId) {
    }
}