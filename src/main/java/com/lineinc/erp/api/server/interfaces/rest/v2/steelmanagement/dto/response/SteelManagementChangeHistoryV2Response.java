...
@Schema(description = "강재수불부 V2 변경 이력 응답")
public record SteelManagementChangeHistoryV2Response(
        ...
        @Schema(description = "변경 유형", example = "입고") String type,
        @Schema(description = "변경 유형 코드", example = "INCOMING") SteelManagementChangeHistoryV2Type typeCode,
        @Schema(description = "수정 가능 여부", example = "true") Boolean isEditable) {

    public static SteelManagementChangeHistoryV2Response from(
            final SteelManagementChangeHistoryV2 history,
            final Long loginUserId) {
        return new SteelManagementChangeHistoryV2Response(
                history.getId(),
