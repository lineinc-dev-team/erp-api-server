...
public interface SiteRepositoryCustom {
     ...
     */
    Page<SiteResponse> findAll(SiteListRequest request, Pageable pageable,
            List<Long> accessibleSiteIds);

    /**
     * 엑셀 다운로드용 - 페이지네이션 없이 현장 목록 전체 조회
     */
    List<Site> findAllWithoutPaging(SiteListRequest request, Sort sort,
            List<Long> accessibleSiteIds);

    /**
     * 대시보드에서 사용할 진행/최근 종료 현장 목록을 조회합니다.
     */
    List<Site> findSitesForDashboard(OffsetDateTime endedAtThreshold,
            OffsetDateTime currentDateTime, List<Long> accessibleSiteIds);
}
