...
public interface OutsourcingCompanyContractConstructionGroupRepository
    ...
    /**
     * 외주업체 계약 ID 목록과 키워드로 공사항목 그룹 정보를 페이징하여 조회합니다.
     * 키워드는 항목명으로 검색됩니다.
     */
    @Query("""
            SELECT g FROM OutsourcingCompanyContractConstructionGroup g \
            WHERE g.outsourcingCompanyContract.id IN :contractIds \
            AND (:keyword IS NULL OR g.itemName LIKE %:keyword%) \
            AND g.deleted = false""")
    Page<OutsourcingCompanyContractConstructionGroup> findByContractIdsAndKeyword(
